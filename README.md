...
- создает сущности `sensor`, `binary_sensor`, `switch`, `number`, `select`, `button` и `text`;
- отображает редактируемые enum-контролы Wiren Board как выпадающие списки;
- позволяет инвертировать состояние отдельных бинарных сенсоров через настройки интеграции;
- распознаёт типы контролов `gas` и `water` как счётчики с единицей `m³`;
- скрывает системные устройства в списке выбора по умолчанию;
- проставляет классы и единицы измерения для основных сенсоров;
- нормализует варианты единиц Wiren Board (`deg C`, `%, RH`, `m^3`) в форматы Home Assistant;
- показывает локализованные подписи read-only enum-контролов вместо числовых кодов;
...
- создаёт отдельное устройство `SMS Gateway` с состоянием обязательного WB-скрипта, последней попыткой и событием результата;
- показывает постоянное уведомление Home Assistant, если Wiren Board сообщил об ошибке отправки SMS.
...
При запуске Home Assistant интеграция дожидается полного подключения к MQTT
Wiren Board. Если соединение недоступно, действие завершится с понятной ошибкой,
а не потеряет первую SMS без уведомления.

Контролы `last_sent_time`, `last_message_text`, `last_recipient_number` и
`last_result` объединяются в отдельное логическое устройство `SMS Gateway`.
Интеграция создаёт событие `Отправка SMS` типов `sent` и `failed`; данные события
содержат получателя, текст, результат и время. При `failed` дополнительно создаётся
постоянное уведомление Home Assistant. Старое retained-состояние при запуске
используется только как исходное и не вызывает ложного уведомления.

## Логические устройства

Если бинарный вход отображает обратное состояние, откройте настройки интеграции
и выберите `Инвертировать бинарные сенсоры`. Отметьте нужные сенсоры и сохраните
настройки — состояния `Включено` и `Выключено` для них поменяются местами.

Пункт `Выбрать элементы` после первоначальной установки намеренно отсутствует.
Добавленные сущности больше нельзя случайно убрать из конфигурации, оставив в
реестре Home Assistant пустое устройство.

Для сенсоров объёма единицы `m^3` и `m3` автоматически исправляются на `m³`.
При создании или редактировании группы выберите в поле `Тип устройства` значение
`Счётчик газа` или `Счётчик воды`. Существующий маппинг интеграции назначит
соответствующий класс Home Assistant и включит долговременную статистику. Если
тип уже задан непосредственно у контрола Wiren Board как `gas` или `water`, он
также будет распознан автоматически.

Названия сущностей можно менять штатно в Home Assistant: откройте сущность,
перейдите в её настройки и измените поле имени. Заданное имя также используется
в списке инверсии бинарных сенсоров.

Если элемент исключён из группы, интеграция удаляет его сущность и очищает пустое
устройство, а не переносит элемент обратно в автоматически созданное устройство.
Если существующий элемент добавлен в другую логическую группу, интеграция сама
переносит его сущность в новое устройство и удаляет прежнее пустое устройство.
Устройства, уже созданные предыдущими версиями, можно убрать через пункт настроек
`Удалить лишние объекты`. Элементы действующих групп в этот список не попадают.
