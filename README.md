...
После каждой принятой команды скрипт на Wiren Board должен очищать контрол.
Благодаря этому два полностью одинаковых SMS подряд обрабатываются как отдельные
изменения `whenChanged`. Интеграция намеренно публикует только одно значение:
быстрая пара «очистить → отправить» может быть объединена драйвером Wiren Board.
Вызовы SMS выполняются последовательно, и действие завершается после короткой
паузы на очистку контрола — поэтому одинаковые сообщения можно отправлять подряд.
